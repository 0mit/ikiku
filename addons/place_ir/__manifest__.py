...
data/ is a place BUNDLE (place_graph/tools/bundle.py): CSV files and a manifest with their
checksums, sources and licences, made outside Odoo. place_graph's bulk loader takes it in on
install and on every update of this module -- in seconds, only the difference, and never over
what staff decided here (the overlay).
...
Open Database Licence (ODbL) 1.0: https://www.openstreetmap.org/copyright. The files in data/
are a derived database and carry that licence (manifest.json says so too); the code is
AGPL-3.0. Anything built on them has to keep saying both.
